...
function installQuestions() {
    ...
    echo -e "  ╭──────────────────────────────────────────────────────╮"
		echo -e "  │                                                      │"
		echo -e "  │                ${WHITE}✻  WireShield${NC} ${GRAY}v3.0.8${NC}                  │"
		echo -e "  │                                                      │"
...
function _ws_install_2fa_rules_service() {
	# Install the firewall helper script and a systemd service that adds the
	# iptables --match-set rules AFTER wg-quick brings up the interface.
	#
	# Why not PostUp? On Ubuntu 22.04+ / 26.04 the default iptables is the
	# nf_tables backend (iptables-nft). When it encounters --match-set it calls
	# into libipset (user-space) which opens a NETLINK_GENERIC socket to validate
	# the set exists. wg-quick's AppArmor-confined execution context (Ubuntu 26.04)
	# returns EPERM for that socket open even when ip_set/xt_set modules are loaded
	# and the sets already exist — resulting in "Can't open socket to ipset".
	# Running identical iptables commands from an unconfined systemd service unit
	# succeeds. After=wg-quick@<iface> guarantees PostUp has already created the
	# WS_2FA_REDIRECT / WS_2FA_PORTAL chains before we reference them.
	local iface="$1"
	local unit_dir="${WS_TEST_SYSTEMD_DIR:-/etc/systemd/system}"
	local fw_script="${WS_TEST_FW_SCRIPT:-/etc/wireguard/wireshield-fw.sh}"
	local iptables_bin ip6tables_bin
	iptables_bin=$(command -v iptables 2>/dev/null || echo /usr/sbin/iptables)
	ip6tables_bin=$(command -v ip6tables 2>/dev/null || echo /usr/sbin/ip6tables)

	# Generate the helper script with all four match-set rules baked in.
	# Use -I FORWARD 1 so the ACCEPT rule is inserted BEFORE WS_2FA_PORTAL.
	cat > "${fw_script}" << SCRIPT_EOF
#!/bin/bash
# WireShield 2FA match-set firewall rules — generated by installer, do not edit.
add() {
    ${iptables_bin} -t nat -A PREROUTING -i ${iface} -p tcp --dport 80 -m set ! --match-set ws_2fa_allowed_v4 src -j WS_2FA_REDIRECT 2>/dev/null || true
    ${iptables_bin} -I FORWARD 1 -i ${iface} -m set --match-set ws_2fa_allowed_v4 src -j ACCEPT 2>/dev/null || true
    ${ip6tables_bin} -t nat -A PREROUTING -i ${iface} -p tcp --dport 80 -m set ! --match-set ws_2fa_allowed_v6 src -j WS_2FA_REDIRECT6 2>/dev/null || true
    ${ip6tables_bin} -I FORWARD 1 -i ${iface} -m set --match-set ws_2fa_allowed_v6 src -j ACCEPT 2>/dev/null || true
}
del() {
    ${iptables_bin} -t nat -D PREROUTING -i ${iface} -p tcp --dport 80 -m set ! --match-set ws_2fa_allowed_v4 src -j WS_2FA_REDIRECT 2>/dev/null || true
    ${iptables_bin} -D FORWARD -i ${iface} -m set --match-set ws_2fa_allowed_v4 src -j ACCEPT 2>/dev/null || true
    ${ip6tables_bin} -t nat -D PREROUTING -i ${iface} -p tcp --dport 80 -m set ! --match-set ws_2fa_allowed_v6 src -j WS_2FA_REDIRECT6 2>/dev/null || true
    ${ip6tables_bin} -D FORWARD -i ${iface} -m set --match-set ws_2fa_allowed_v6 src -j ACCEPT 2>/dev/null || true
}
case "\$1" in add|del) "\$1";; *) echo "Usage: \$0 {add|del}" >&2; exit 1;; esac
SCRIPT_EOF
	chmod 700 "${fw_script}"

	mkdir -p "${unit_dir}"
	cat > "${unit_dir}/wireshield-2fa-rules.service" <<- UNIT_EOF
		[Unit]
		Description=WireShield 2FA iptables match-set rules for ${iface}
		After=wg-quick@${iface}.service wireshield-ipsets.service
		PartOf=wg-quick@${iface}.service

		[Service]
		Type=oneshot
		RemainAfterExit=yes
		ExecStart=${fw_script} add
		ExecStop=${fw_script} del

		[Install]
		WantedBy=multi-user.target
	UNIT_EOF

	# Drop-in so wg-quick@<iface> pulls in and starts wireshield-2fa-rules
	# on every boot / wg-quick start without manual intervention.
	mkdir -p "${unit_dir}/wg-quick@${iface}.service.d"
	cat > "${unit_dir}/wg-quick@${iface}.service.d/wireshield-2fa-rules.conf" <<- 'DROPIN_EOF'
		[Unit]
		Wants=wireshield-2fa-rules.service
	DROPIN_EOF

	systemctl daemon-reload 2>/dev/null || true
	systemctl enable wireshield-2fa-rules.service 2>/dev/null || true
}
...
PostUp = iptables -C FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || iptables -I FORWARD 1 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
...
PostDown = iptables -D FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || true
...
net.ipv4.tcp_mtu_probing = 1" >/etc/sysctl.d/wg.conf
	...
	if [[ ${OS} == 'alpine' ]]; then
		...
		# Install the service that adds --match-set iptables rules after wg-quick
		# starts. These rules cannot run in PostUp on Ubuntu 26.04: iptables-nft
		# opens a user-space libipset netlink socket to validate set existence,
		# and wg-quick's AppArmor-confined context blocks that socket open.
		_ws_install_2fa_rules_service "${SERVER_WG_NIC}"
		...
		if ! systemctl is-active --quiet "wg-quick@${SERVER_WG_NIC}"; then
			...
			if [[ "${_WS_WG_BACKEND}" == "unavailable" ]] || \
			   echo "${_wg_journal}" | grep -qiE "protocol not supported|operation not supported|unknown device type|cannot find device"; then
				...
				echo -e "${ORANGE}Fix:   WireShield v3.0.8+ moves --match-set rules to wireshield-2fa-rules.service${NC}"
				...
				echo -e "${ORANGE}       If xt_set is missing: install linux-modules-extra-$(uname -r)${NC}"
...
function uninstallWg() {
	...
	if [[ $REMOVE == 'y' ]]; then
		...
		if [[ ${OS} == 'alpine' ]]; then
			...
			systemctl disable --now wireshield-2fa-rules.service 2>/dev/null || true
			rm -f "/etc/systemd/system/wg-quick@${SERVER_WG_NIC}.service.d/wireshield-ipsets.conf" 2>/dev/null || true
			rm -f "/etc/systemd/system/wg-quick@${SERVER_WG_NIC}.service.d/wireshield-2fa-rules.conf" 2>/dev/null || true
...
function _ws_header() {
	...
	# Brand line
	echo ""
	echo -e "  ${WHITE}✻  WireShield${NC}  ${GRAY}v3.0.8${NC}   ${DIM}Zero-trust WireGuard VPN${NC}"
	_ws_ui_divider
	echo ""
